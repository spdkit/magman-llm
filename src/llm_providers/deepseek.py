...
class DeepSeekProvider(BaseLLMProvider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.api_key:
            raise ValueError(
                "DeepSeek API key not provided, please set --api-key parameter or DEEPSEEK_API_KEY environment variable"
            )
    ...
    def _init_client(self) -> bool:
        ...
        try:
            ...
            if self.debug:
                print(
                    f"Initialized DeepSeek provider: model={self.model}, base_url={self.base_url}"
                )
