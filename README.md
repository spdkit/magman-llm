...
### Cross-system Transfer Validation Data

The `transfer_validation/` directory contains normalized reviewer-facing prediction tables and per-run evaluation metrics for the three system transfer/adaptation tests described in the manuscript (Fe₃O₄, γ-Fe₂O₃, Mn₂O₃). These tables are generated by `scripts/normalize_public_release.py` from formal prediction assets without any LLM/API calls, and include:

- `normalized_predictions.csv` — per-sample predictions with both task-specific labels and manuscript event-positive semantics
- `normalized_metrics.csv` — per-run AUC, accuracy, and confusion-matrix numbers under both label conventions
- `table2_cross_system_auc.csv` — compact source data for the cross-system prompt-tier AUC table
- `table3_mn2o3_prompt_adaptation.csv` — compact source data for the Mn₂O₃ prompt-adaptation table

Process-only fields (absolute paths, LLM reasoning text, API timing, error logs, console output, raw OSZICAR contents) are excluded. See `transfer_validation/README.md` for label semantics and the event-positive normalization convention.
